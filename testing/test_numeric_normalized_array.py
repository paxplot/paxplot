...
def test_numeric_normalized_array():
    raw = [1, 2, 3]
    obj = NumericNormalizedArray(values=raw)

    expected = np.array([-1.0, 0.0, 1.0])
    result = obj.values_normalized
...
def test_append_array():
    obj = NumericNormalizedArray(values=[1, 2])
    obj.append_array([3, 4])

    # Should now contain [1, 2, 3, 4]
    assert obj.values == [1, 2, 3, 4]
    np.testing.assert_array_almost_equal(
        obj.values_normalized, np.array([-1.0, -0.3333, 0.3333, 1.0]), decimal=4
    )


def test_remove_indices():
    obj = NumericNormalizedArray(values=[10, 20, 30, 40])
    obj.remove_indices([1, 3])

    # Should now contain [10, 30]
    assert obj.values == [10, 30]
    np.testing.assert_array_almost_equal(
        obj.values_normalized, np.array([-1.0, 1.0])
    )
...
def test_constant_array_normalization():
    obj = NumericNormalizedArray(values=[7, 7, 7])
    result = obj.values_normalized
    np.testing.assert_array_almost_equal(result, np.zeros(3))

def test_set_custom_bounds():
    arr = [1.0, 2.0, 3.0]
    obj = NumericNormalizedArray(values=arr)
    obj.set_custom_bounds(min_val=0.0, max_val=10.0)
    assert np.allclose(obj.values_normalized, [-0.8, -0.6, -0.4])
...
def test_update_array():
    ...
    np.testing.assert_array_almost_equal(
        obj.values_normalized, np.array([-1.0, 0.0, 1.0])
    )
