...
class TestMatrix:
    ...
    def test_column_type_inference_mixed_types_raises_error(self):
        ...
        with pytest.raises(ValueError, match="mixed numeric and categorical data"):
            Matrix(data)
    ...
    def test_column_type_inference_all_nan_numeric(self):
        ...
        matrix = Matrix(data)
        assert matrix.get_column_type(0) == ColumnType.CATEGORICAL  # All NaN defaults to categorical
        assert matrix.get_column_type(1) == ColumnType.CATEGORICAL
    ...
    def test_infer_column_type_with_nan(self):
        """Test _infer_column_type with NaN values."""
        data = [[1.0, 'A']]
        matrix = Matrix(data)
        
        # Column with numeric and NaN values should be numeric
        column_data = [1.0, float('nan'), 3.0, None]
        column_type = matrix._infer_column_type(column_data)
    ...
    def test_infer_column_type_mixed_types_raises_error(self):
        """Test _infer_column_type with mixed types raises error."""
        data = [[1.0, 'A']]
        matrix = Matrix(data)
        
        # Column with both numeric and string values (excluding NaN) should raise error
        column_data = [1.0, 'A', 3.0, None]  # NaN is ignored, but 1.0 and 'A' conflict
        with pytest.raises(ValueError, match="mixed numeric and categorical data"):
            matrix._infer_column_type(column_data)
